...
if __name__ == '__main__':
    ...
    # 获取网络
    net = calling_DNLCNN_from_tar(tar_path, cls_num=7)
    ...
    path_list = os.listdir('emo3')
    path_list = [os.path.join('emo3',i) for i in path_list]
    ...
    for img in path_list:
        if method != 'pro' and method != 'random':
            ...
            generate_html(G, root, arch, dataset, cam_method, img, net, wnids, num_cls,
                          output_dir, html_output, None, 's', 'simple')
            generate_html(G, root, arch, dataset, cam_method, img, net, wnids, num_cls,
                          output_dir, html_output, None, 'w', 'w')
            generate_html(G, root, arch, dataset, cam_method, img, net, wnids, num_cls,
                          output_dir, html_output, None, 'c', 'complex')
        else:
