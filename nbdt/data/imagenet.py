...
__all__ = names = ('TinyImagenet200', 'Imagenet1000','Fashion10')
...
class DIYdataset(Dataset):
    ...
    def __init__(self, root="./data", *args, train=True, download=False, **kwargs):
        ...
        if download:
            self.download(root=root)
        dataset = _DIYdatasetTrain if train else _DIYdatasetVal
        self.root = root
        self.dataset = dataset(root, *args, **kwargs)
        self.classes = self.dataset.classes
        self.class_to_idx = {cls: i for i, cls in enumerate(self.classes)}
...
class _DIYdatasetTrain(datasets.ImageFolder):
    def __init__(self, root="./data", *args, **kwargs):
        super().__init__(os.path.join(root, "PLANE/train"), *args, **kwargs)


class _DIYdatasetVal(datasets.ImageFolder):
    def __init__(self, root="./data", *args, **kwargs):
        super().__init__(os.path.join(root, "PLANE/val"), *args, **kwargs)


class Fashion10(Dataset):
    """ImageNet dataloader"""

    dataset = None

    def __init__(self, root="/data/LZL", *args, train=True, download=False, **kwargs):
        super().__init__()
        if download:
            self.download(root=root)
        dataset = _Imagenet10Train if train else _Imagenet10Val
        self.root = root
        self.dataset = dataset(root, *args, **kwargs)
        self.classes = self.dataset.classes
        self.class_to_idx = {cls: i for i, cls in enumerate(self.classes)}
...
class _Fashion10Train(datasets.ImageFolder):
    def __init__(self, root="/data/LZL", *args, **kwargs):
        super().__init__(os.path.join(root, "Fashion10/train"), *args, **kwargs)


class _Fashion10Val(datasets.ImageFolder):
    def __init__(self, root="/data/LZL", *args, **kwargs):
        super().__init__(os.path.join(root, "Fashion10/val"), *args, **kwargs)
