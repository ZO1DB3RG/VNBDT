...
class DFL_VGG16(nn.Module):
	def __init__(self, k = 10, nclass = 200):
		...
		# 返回带bn层的VGG16特征提取部分【后面是poling和classification】
		...
		"""
		将vgg16bn分为了conv1-4和conv5两个部分，conv5这个部分就是整个模型的left G-stream部分，
		即正常的vgg往后的部分，用于抽取全局特征
		"""
		#中间用1*1的卷积核，Conv6即要学习的Cx1x1的卷积核，一共kM个，M为类别数，k为每个类别要检测part数，构成middle P-Stream部分
